    ...
    doAjaxCall(url, data, false, function (html) {
        ...
        if ($.isEmptyObject(html)) {
            ...
            if (validateTime == 1) {
                ...
                $.each(html, function (i, item) {
                    if (counter > i) {
                        data += '<li><a rel="external"  href="merchandise_category.html?location=' + item.locationId + '&transferId=' + featureRelId + '&touchId=' + userSiteId + '" data-transition="flip" >' + item.city + '</a></li>';
                    }
                });
                data += '</ul>';
...
function merchandiseItemInfo() {
    ...
    var url = baseUrl + 'web/web/merchcategoryItemdetailsinfo/' + location + '/' + featureRelId + '/' + userSiteId + '/' + categoryId + '/' + itemId;
    doAjaxCall(url, data, false, function (html) {
...
function submitItem(transferId, touchId, location) {
    ...
    doAjaxCall(url, data, true, function (html) {
        if (html == 1) {
            window.location.href = 'merchandise_add_cart.html?transferId=' + transferId + '&touchId=' + touchId + '&location=' + location;
        } else {
...
function merchandisecheckoutcart() {
    ...
    var data = '';
    var validationData;
    url = baseUrl + 'web/web/merchGetPayInfo/' + sessionid + '/' + featureRelId + '/' + userSiteId + '/' + location;
    //alert(url);
    doAjaxCall(url, '', false, function (validationData) {
        ...
        if ($.isEmptyObject(validationData)) {
            ...
            url2 = baseUrl + 'web/web/merchgetcartInfo/' + sessionid + '/' + featureRelId + '/' + userSiteId + '/' + location;
